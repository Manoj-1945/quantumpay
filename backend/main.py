...
class IBMQiskitEngine:
    ...
    def _run_qiskit_sync(self, needed_chunks):
        ...
        try:
            # 1. Authenticate with IBM Cloud (automatically resolves CRN and instances)
            service = QiskitRuntimeService(channel="ibm_cloud", token=self.ibm_token)
        except Exception as e:
            msg = str(e)
            if "instance" in msg.lower() or "crn" in msg.lower():
                print("[IBM POOL] CRITICAL ERROR: Your IBM Cloud account is missing a Quantum Workspace!")
                print("[IBM POOL] Fix this by going to: https://cloud.ibm.com/catalog/services/ibm-quantum-workspace")
                print("[IBM POOL] Create a FREE instance, then restart the server.")
            else:
                print(f"[IBM POOL] Qiskit Auth Error: {msg}")
            return 0 # Trigger OS fallback
        ...
        try:
            ...
            for bitstring in counts.keys():
                val = int(bitstring, 2)
                entropy_hex += hex(val)[2:]
                
            # Split into 32-byte (64 hex chars) chunks
            chunks = [entropy_hex[i:i+64] for i in range(0, len(entropy_hex), 64)]
            valid_chunks = [c for c in chunks if len(c) == 64]
            
            print(f"[IBM POOL] Job complete! Successfully extracted {len(valid_chunks)} quantum chunks.")
            return valid_chunks
            
        except Exception as e:
            print(f"[IBM POOL] IBM Job Error: {str(e)}")
            return 0
    ...
    async def run_monthly_harvest(self):
        ...
        print(f"[IBM POOL] Starting monthly harvest for {current_month} (Target: {self.monthly_target} chunks)")
        
        harvested = 0
        needed = self.monthly_target # always harvest 300 right now for testing
        
        # Run the Qiskit job in a background thread to prevent blocking the FastAPI event loop
        loop = _aio.get_running_loop()
        ibm_chunks = await loop.run_in_executor(None, self._run_qiskit_sync, needed)
        
        if ibm_chunks and isinstance(ibm_chunks, list):
            async with db_pool.acquire() as conn:
                for chunk_hex in ibm_chunks[:needed]:
                    await conn.execute(
                        "INSERT INTO ibm_entropy_pool (entropy_hex, used, harvest_month) VALUES ($1, 0, $2)",
                        chunk_hex, current_month
                    )
                    harvested += 1
            print(f"[IBM POOL] Monthly harvest complete: {harvested} chunks added to DB pool from IBM Quantum")
        
        # OS-CSPRNG Fallback (if IBM failed or didn't return enough)
        if harvested < needed:
            remaining = needed - harvested
            print(f"[IBM POOL] IBM harvest got {harvested} chunks — filling remaining {remaining} with OS-CSPRNG fallback")
            fallback_count = 0
            async with db_pool.acquire() as _fi_conn:
                for _fi in range(remaining):
                    ...
                    fallback_count += 1
            print(f"[IBM POOL] OS-CSPRNG fallback fill complete: {fallback_count} chunks stored")
    ...
    def get_entropy_status(self) -> dict:
        return {
            "status": "POOL_ACTIVE" if self.ibm_token else "DISABLED",
